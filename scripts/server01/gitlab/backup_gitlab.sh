...
GITLAB_LOCAL_BACKUP_PATH="/opt/gitlab/backups"
GITLAB_MOUNT_CONFIG_PATH="/opt/gitlab/server/config"
DATE_ID="$(date +'%Y%m%d_%H%M%S')"
TAR_PRE_STRING="backup_gitlab_"
TAR_POST_STRING=".tar"
...
function backupandupload() {
	...
	backup_name="$(docker exec -t $GITLAB_CONTAINER_NAME ls -A1 $GITLAB_CONTAINER_BACKUP_PATH/ | sort -n | tail -n 1)"
	real_file_name=$(printf $backup_name | tr -d '\r')
	container_backup_file=$GITLAB_CONTAINER_BACKUP_PATH/$real_file_name
	docker cp $GITLAB_CONTAINER_NAME:$container_backup_file $GITLAB_LOCAL_BACKUP_PATH/$DATE_ID/
...
function checkcontainertodelete() {
	...
	# 先計算總數量
	number=$(docker exec $GITLAB_CONTAINER_NAME sh -c "ls -la $GITLAB_CONTAINER_BACKUP_PATH | grep .tar | wc -l")
		...
		docker exec $GITLAB_CONTAINER_NAME sh -c "find $GITLAB_CONTAINER_BACKUP_PATH -name '*.tar' | sort -n | head -n $del | xargs rm -rf"
		echo "Delete finish"
        else
                echo "Exist files smaller then $FILE_MAX , no action"
        fi
