...
backup_name="mysql"
backup_from_container="mysql"
backup_to="/mnt/devops_backup/mysql"
...
backup_data() {
    ...
    source "${env_file}"
    docker exec ${backup_from_container} mysqldump -u root --password=${MYSQL_ROOT_PASSWORD} --all-databases | gzip > "${backup_to}/backup_${backup_name}_$(date -u +%Y%m%d)_$(date -u +%H%M%S).sql.gz" || handle_error "Failed to backup ${backup_name} from container ${backup_from_container}"
